function net = udntrain(net, udn_index, AfterNMS, x, y, opts, test_x, test_y, Test_Boxes, Test_Frame, Train_Boxes)%, tx, ty
y = single(y);
net.batchMaxl  = 10;
...
for i = 1 : opts.numepochs
    ...
    for l = 1 : numbatches
        ...
        net.batchl = l;
        net = udnff(net, batch_x);
        net = udnbp(net, batch_y);
        net = udnapplygrads(net, opts);
...
%     end
    ...
    ModelFname = ['.\New_UDNModel\UDN_Model_iter' num2str(udn_index) '.' num2str(i) '.mat'];
    udn_model = copycnnmodel(net);
